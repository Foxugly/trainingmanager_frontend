...
**`teams-form`, `teams-detail` and `events-detail` are thin parents** that delegate to extracted child components (the big detail/form pages were decomposed): teams-form hosts `app-team-slots-editor`, `app-team-managers`, `app-team-place-pool` (the sport tab itself is inline — a `sport_ids` multiselect + `default_sport_id` select); teams-detail hosts `app-team-requests` (the manager join-requests + invitations tab, emits `(countChange)` for the tab badge + `(membershipsChanged)` after an accept) alongside `app-team-discussions`/`app-team-stats`; events-detail hosts `app-event-roti`, `app-event-rsvp`, `app-event-training` (rounds + exercises editor), plus the already-extracted `app-attendance-manager` / `app-attachment-list`. **Child↔parent contracts** (stateless-picker style): `app-team-place-pool` emits `(placeIdsChange)`/`(defaultPlaceIdChange)`/`(poolChange)` (the parent keeps the form controls + feeds the pool to the slots editor); `app-event-training` takes `event`/`team`/`canManage`/`restrictedViewer` and emits `(stateChange)`=`{totalDistance,loading}` (the parent owns the header total + the `event.total` auto-patch) and `(reloadRequested)` after a mutation (the parent refetches the event and feeds a fresh one back down). The training editor builds its rounds+exercises from the event's embedded **`rounds_detail`** (one request on retrieve) — no per-round/per-exercise fetch.

**Multi-sport teams:** a team practises several sports (`team.sports`, each `TeamSportRead` with `is_default`); write via `sport_ids` + `default_sport_id`. `team.sport` (read) is the default. Per-session sport lives on `event.sport`, per-slot on the slot's sport. The session-sport select (events-form), the per-slot sport column (slots editor) and the event-detail sport badge only show when the team has >1 sport. The events-detail catalog pickers scope to `event.sport`. Backend design: `trainingmanager_server/docs/multi-sport-teams-design.md`.
...
- **`role="menu"` contract**: any element with `role="menu"` MUST support arrow-key navigation (ArrowUp/Down/Home/End) + roving tabindex. See `language-switcher.component.ts` for the reference implementation. Conversely, don't slap `role="menu"`/`role="menuitem"` on a widget that's just a grid of buttons — the team-discussions emoji picker is a plain `role="group"` of Tab-navigable `<button>`s (no menu promise, no arrow-key requirement).
- **team-discussions data load**: topic loading uses `toObservable(teamId) → switchMap` (cancels the in-flight request on team switch), the preferred pattern over a side-effecting `effect()` for data. Emoji insertion parses the editor HTML with an inert `<template>` and appends the emoji as a text node (no fragile HTML regex).
- **Rich-text editor / Quill**: never use `p-editor` (or import `quill.snow.css` globally) directly. Use `app-rich-editor` (`src/app/shared/ui/rich-editor/`) — a transparent `ControlValueAccessor` wrapper that keeps `formControlName`/`[formControl]` working and ships Quill's ~24 kB CSS in a lazy chunk (via `ViewEncapsulation.None` component style) instead of the initial bundle. Importing `quill/dist/quill.snow.css` in `src/styles.scss` would put it back in the initial bundle and blow the 1 MB budget. The wrapper's scss intentionally exceeds the per-component-style warning (4 kB); the `anyComponentStyle` error ceiling in `angular.json` is raised to 28 kB to allow this one vendored file.
